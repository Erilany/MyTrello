...
**Action vocale**
Commande reconnue par le moteur vocal qui déclenche une opération dans MyTrello (création, déplacement, navigation, etc.). Distinct d'un *texte dicté* qui lui est inséré dans un champ de saisie.
...
**Bibliothèque**
Panel latéral permettant de sauvegarder des éléments MyTrello (cartes, catégories, sous-catégories) comme *modèles* réutilisables. Les modèles peuvent être glissés depuis la bibliothèque vers le tableau pour en créer des copies.
...
**Carte / Card**
Élément de niveau 1 dans la hiérarchie MyTrello. Représente un projet. Contient des *catégories*. Appartient à une *colonne*.

**Catégorie / Category**
Élément de niveau 2 dans la hiérarchie MyTrello. Appartient à une *carte*. Contient des *sous-catégories*. Peut être déplacée vers une autre carte.
...
**Copier / Coller vocal**
Mécanisme permettant de copier un email (Outlook ou Gmail) en mémoire tampon via une commande vocale, puis de le coller dans un élément MyTrello via une autre commande vocale.
...
**Electron**
Framework permettant de créer des applications desktop multiplateformes avec des technologies web (HTML, CSS, JavaScript). Utilisé comme base de MyTrello. Composé d'un *Main Process* et d'un *Renderer Process*.
...
**Exchange on-premise**
Serveur Microsoft Exchange installé et géré en interne par une entreprise, par opposition à Exchange Online (cloud). Nécessite l'utilisation d'EWS dans MyTrello.
...
Structure d'organisation des données dans MyTrello : *Carte* (niveau 1) → *Catégorie* (niveau 2) → *Sous-catégorie* (niveau 3). Chaque niveau peut contenir des éléments du niveau inférieur.
...
Mécanisme de communication entre le *Main Process* et le *Renderer Process* d'Electron. Dans MyTrello, tous les appels à la base de données et aux APIs externes passent par IPC.

**Indicateur RDV du jour**
Badge affiché dans le header de MyTrello indiquant le nombre de rendez-vous du jour dans le calendrier Outlook, filtré par les tags actifs.
...
**Lien email**
Association entre un email (Outlook ou Gmail) et un élément MyTrello (carte, catégorie ou sous-catégorie). Stocké dans la table `email_links`.
...
**Mapping tags**
Règle de correspondance entre un tag de messagerie (catégorie Outlook, libellé Gmail, tag calendrier) et une étiquette MyTrello. Configuré dans l'interface de synchronisation.
...
**Microsoft Graph API**
API REST de Microsoft permettant d'accéder aux services Microsoft 365 (Outlook, Exchange Online, Calendrier). Mode principal d'intégration Outlook dans MyTrello.
...
**Niveau**
Position d'un élément dans la hiérarchie MyTrello. Niveau 1 = Carte, Niveau 2 = Catégorie, Niveau 3 = Sous-catégorie.
...
**OAuth 2.0**
Protocole d'authentification et d'autorisation utilisé pour les connexions Outlook (Microsoft) et Gmail (Google). Permet à MyTrello d'accéder aux emails de l'utilisateur sans stocker son mot de passe.
...
Élément de niveau 3 dans la hiérarchie MyTrello. Appartient à une *catégorie*. Peut être déplacée vers une autre catégorie.

**SQLite**
Base de données relationnelle légère stockée localement dans un fichier. Utilisée pour persister toutes les données MyTrello (tableau, cartes, paramètres, cache calendrier, etc.).

**Synchronisation bidirectionnelle**
Mécanisme de sync où les changements se propagent dans les deux sens : messagerie → MyTrello ET MyTrello → messagerie. Configuré par *mapping tags*.
...
**Tableau / Board**
Espace de travail principal de MyTrello. Contient des *colonnes*, chacune contenant des *cartes*.

**Tag (calendrier Outlook)**
Catégorie Outlook associée à un événement calendrier. Utilisée pour filtrer les événements dans le module calendrier de MyTrello.
...
*MyTrello — Glossaire — 23 février 2026*
